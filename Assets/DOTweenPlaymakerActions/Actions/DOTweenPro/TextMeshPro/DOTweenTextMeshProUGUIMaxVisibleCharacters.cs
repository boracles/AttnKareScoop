...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Changes the target's maxVisibleCharacters to the given value. NOTE: if you didn't set the maxVisibleCharacters property before starting the tween, TextMesh Pro will automatically set the starting value to 0 (because the property is activated only the first time it's used).")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenTextMeshProUGUIMaxVisibleCharacters : FsmStateAction
    {
        [RequiredField]
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<TextMeshProUGUI>().DOMaxVisibleCharacters(to.Value, duration.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween TextMeshProUGUI MaxVisibleCharacters");
