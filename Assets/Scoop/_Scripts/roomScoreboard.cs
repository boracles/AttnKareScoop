...
public class roomScoreboard : MonoBehaviour
{
    ...
    void ballUpdate(GameObject ball)
    {
        ...
        if (ball.GetComponent<roomBall>().ScoreCheck && !successBalls.Contains(ball.name))
        {
            successBalls.Add(ball.name);
            score++;
        }
        // When ball hits floor(outside of container) and was already listed
        else if (!ball.GetComponent<roomBall>().ScoreCheck && successBalls.Contains(ball.name))
        {
