...
public class easyBall : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if(collision.gameObject.tag == "Floor" || collision.gameObject.tag == "StartContainer" || collision.gameObject.tag == "Desk")
        {
            ...
            {
                resetBall(gameObject);
                GetComponentInParent<easyScoreboard>().totalDrops++;
            }
        ...
        if (collision.gameObject.tag == "Boundary")
        {
            ...
            resetBall(gameObject);
            ScoreCheck = false;
            if(GetComponentInParent<easyScoreboard>().totalDrops > 0) GetComponentInParent<easyScoreboard>().totalDrops--;
        }

        // Calls scoreUpdate function in Scoreboard script every time ball collides with the environment
        GetComponentInParent<easyScoreboard>().scoreUpdate();
    }
