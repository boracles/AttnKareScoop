...
public class easyScoreboard : MonoBehaviour
{
    ...
    public void reset()
    {
        foreach (GameObject ball in clonedBalls)
        {
            GetComponentInChildren<easyBall>().resetBall(ball);
            ball.GetComponent<easyBall>().ScoreCheck = false;
    ...
    void ballUpdate(GameObject ball)
    {
        ...
        if (ball.GetComponent<easyBall>().ScoreCheck && !successBalls.Contains(ball.name))
        {
            successBalls.Add(ball.name);
            score++;
        }
        // When ball hits floor(outside of container) and was already listed
        else if (!ball.GetComponent<easyBall>().ScoreCheck && successBalls.Contains(ball.name))
        {
